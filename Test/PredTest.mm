...
// Weighted prediction from box prediction

- (void) testBoxPredWeighted1 {
  
  uint32_t colortablePixelsPtr[] = {
    0x00000000,
    0x000A3264, // (10, 50, 100)
    0x00FFFFFF,
  };
  ...
  //  uint32_t predPixel = CTI_BoxPredictPixel(ctiStruct,
  //                                           tablePred,
  //                                           0,
  //                                           2,
  //                                           inputPixels,
  ...
  // The only pixels available for this prediction indicate
  // that the value zero is likely.
  
  XCTAssert(predPixel == 0x00000000);

  // Now predict (3,0) given that a new value is available
  ...
  XCTAssert(predPixel == 0x00031020);
  
  return;
}

// Ave of 3 different pixel values with different weights

- (void) testBoxPredWeighted2 {
  ...
  XCTAssert(predPixel == 0x0001050B); // (1 5 11)
  
  return;
}

// Input image that is 2 pixels tall, the average of the 2 row
// values should be the same.

- (void) testBoxPredWeighted3 {
  ...
  // The pixels to predict from at this point:
  
  // (0 0 0) (10 50 100) -> row ave (5 25 50)
  // (0 0 0) (10 50 100) -> row ave (5 25 50)
  
  printf("0x%08X\n", predPixel);
  
  XCTAssert(predPixel == 0x00051932); // (5 25 50)
  
  return;
}

// Neighbor prediction based on pred err of neighbors
// values should be the same.

- (void) testNeighborPredGradclamp1 {
  ...
  // Prediction of (2,0) has only L neighbor, so the
  // pred error is based only on the neighbor at the
  // same H value.
  ...
  XCTAssert(predPixel == 0x00000000); // (0 0 0)
  ...
  // Prediction of (2,1) can examine the U neighbor
  // and the L neighbor. The prediction is a simple
  // mix of the 2 values.
  ...
  XCTAssert(predPixel == 0x00FFFFFF); // gradclamp return pixel above in this case
  
  return;
}

// Another trivial gradclamp pattern, when no H or V axis is detected

- (void) testNeighborPredGradclamp2 {
  ...
  // Prediction of (2,0) has L, DL, D (gradclamp)
